...
export default function AssignmentsPage() {
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      if (!qtPersonQ || qtPersonQ.length < 2) {
        setQtPeople([]);
        return;
      }
      try {
  ...
  const onTransfer = async (e) => {
    ...
    try {
      await transferAssignment({
        item_id: xferItemId,
        to_person_id: xferTo.id,
        due_back_date: xferDue || null,
        notes: xferNotes || null,
      });
      setXferOpen(false);
      await refreshHistory();
    } catch (e2) {
          ...
          <div className="card-body">
            ...
            <div className="list scroll">
              ...
              {people.map((p) => (
                <button
                  key={p.id}
                  className={cls("person-row", pid === p.id && "active")}
                  onClick={() => setPid(p.id)}
        ...
        <div className="modal on">
          <div className="modal-card">
            <div className="modal-head">
              ...
              <button className="icon" onClick={() => setXferOpen(false)}>
                ✕
              </button>
            </div>
                ...
                <div className="grid-two">
                  <div className="control">
                    ...
                    <input
                      ...
                      type="date"
                      value={xferDue}
                      onChange={(e) => setXferDue(e.target.value)}
                    />
                  </div>
                  <div className="control">
                    <label>Notes</label>
